...
public class AnimalLV4 : MonoBehaviour
{
    ...
    {
        // Raycast 확인
        Debug.DrawRay(rigid.position, Vector3.down * 270, new Color(1, 0, 0));
        Attack();
        ShotDelay();
    }
    ...
    {
        // RaycastAll 아래로 확인
        RaycastHit2D[] rayDown = Physics2D.RaycastAll(rigid.position, Vector3.down, 270);
        ...
        for (int i = 0; i < rayDown.Length; i++)
        {
            ...
            if (rayDown[i].collider.gameObject.tag == this.transform.tag)
            {
                ...
                if (isDestroy == 2)
                {
                    ...
                    Instantiate(Animals5[RandomAnimal], new Vector3(nowPosition.x, nowPosition.y + 77, 0), Quaternion.identity);
                    // 동물 생성 가능한 땅 정보 업데이트
